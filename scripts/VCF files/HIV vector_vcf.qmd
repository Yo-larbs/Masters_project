...
Resistant_vector_cells_annovar_csv=read.csv("Res_annovar_output.hg38_multianno.csv")
Susceptible_vector_cells_annovar_csv=read.csv("Sus_annovar_output.hg38_multianno.csv")
```
...
common_mutations_resistant_vector=unique_genes_per_cell%>%dplyr::group_by(Gene.refGene)%>%dplyr::summarise(common=n())%>%arrange(desc(common))
```
...
common_mutations_susceptible_vector=unique_genes_per_cell%>%dplyr::group_by(Gene.refGene)%>%dplyr::summarise(common=n())%>%arrange(desc(common))
```

Here i plot the most mutated genes across cell lines(so in how many cell lines is this gene mutated in?)

```{r}
ggplot(data = common_mutations_susceptible_vector%>%slice_max(common,n=10),aes(x=Gene.refGene,y=common))+
...
```{r}
ggplot(data = common_mutations_resistant_vector%>%slice_max(common,n=10),aes(x=Gene.refGene,y=common))+
  geom_bar(stat="identity",width= 0.05)+
  geom_point(size=3,colour = "grey")+
  theme_minimal()
```
...
```{r}
common_mutations_susceptible_vector[common_mutations_susceptible_vector$Gene.refGene %in%(setdiff(common_mutations_susceptible_vector$Gene.refGene,common_mutations_resistant_vector$Gene.refGene)),]
```
...
```{r}
ggplot(data = common_mutations_susceptible_vector[common_mutations_susceptible_vector$Gene.refGene %in%(setdiff(common_mutations_susceptible_vector$Gene.refGene,common_mutations_resistant_vector$Gene.refGene)),]%>%slice_max(common,n=12),aes(x=Gene.refGene,y=common))+
  geom_bar(stat="identity",width= 0.5)+
  theme_minimal()
```

```{r}
common_mutations_resistant_vector[common_mutations_resistant_vector$Gene.refGene %in%(setdiff(common_mutations_resistant_vector$Gene.refGene,common_mutations_susceptible_vector$Gene.refGene)),]
```

Here i do the same for the other phenotype

```{r}
ggplot(data = common_mutations_resistant_vector[common_mutations_resistant_vector$Gene.refGene %in%(setdiff(common_mutations_resistant_vector$Gene.refGene,common_mutations_susceptible_vector$Gene.refGene)),]%>%slice_max(common,n=12),aes(x=Gene.refGene,y=common))+
  geom_bar(stat="identity",width= 0.5)+
  theme_minimal()
...
```{r}
common_genes_vector=intersect(common_mutations_susceptible_vector$Gene.refGene,common_mutations_resistant_vector$Gene.refGene)
```

```{r}
common_mutations_resistant_vector[common_mutations_resistant_vector$Gene.refGene%in%common_genes_vector,]
```
...
```{r}
common_rates_vector=merge.data.frame(x=common_mutations_resistant_vector[common_mutations_resistant_vector$Gene.refGene%in%common_genes_vector,],y=common_mutations_susceptible_vector[common_mutations_susceptible_vector$Gene.refGene%in%common_genes_vector,],by.x = 'Gene.refGene',by.y = 'Gene.refGene')
```
