...
HIV1_virus_data=read_csv('inputs/hiv1_virus_collated_n0_uncleaned(Sheet1).csv')
```
...
```{r}
apply_plot=function(i){ggplot(data = i,aes(x=infection_volume_ul,y=assay_output,group = interaction(screen,replicate),colour = as.factor(replicate) ,shape = as.factor(screen)))+
  geom_point()+
...
```{r}
ggplot(data = HIV1_virus_data,aes(y=assay_output, x=as.factor(screen_nb),group = screen_nb,))+
  geom_boxplot()+
...
```{r}
ggplot(data = HIV1_virus_data,aes(y=zscore, x=as.factor(screen_nb),group = screen_nb,))+
  geom_boxplot()+
  ggtitle('change between screens')+
  theme_classic()
```
...
```{r}
HIV1_virus_data=HIV1_virus_data%>%group_by(batch,cell_line,screen,screen_nb,replicate)%>%
...
In this i only take the max zscore(as thats what the parameter is) so i get rid of all values that weren't the max zscore (so normally for the min volume or the 5th titre)
due to my earlier code where i got made all values<80% equal to the max, i could get multiple rows where the max zscore is the same so i apply the unique() to get only distinct rows
...
```{r}
susceptible_virus_clusters=ind_coord[(ind_coord$group==2)|(ind_coord$group==4),]
Resistant_virus_clusters=ind_coord[(ind_coord$group==1)|(ind_coord$group==8),]#|(ind_coord$group==6),]
```

```{r}
ggplot(HIV1_virus_data_notable,aes(x=row.names(HIV1_virus_data_notable),y=assay_output,colour = phenotype))+
  geom_point()
```

```{r}
ggplot(HIV1_virus_data_notable,aes(x=row.names(HIV1_virus_data_notable),y=a_log,colour = phenotype))+
  geom_point()
```

```{r}
ggplot(HIV1_virus_data_notable,aes(x=row.names(HIV1_virus_data_notable),y=b_log,colour = phenotype))+
  geom_point()
```

```{r}
ggplot(HIV1_virus_data_notable,aes(x=row.names(HIV1_virus_data_notable),y=area_under_curve,colour = phenotype))+
  geom_point()
```

```{r}
ggplot(HIV1_virus_data_notable,aes(x=row.names(HIV1_virus_data_notable),y=logis_b,colour = phenotype))+
  geom_point()
```
